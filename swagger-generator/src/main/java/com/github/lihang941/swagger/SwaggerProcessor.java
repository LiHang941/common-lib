...
import com.github.lihang941.swagger.base.*;
import com.github.lihang941.swagger.core.*;
...
public abstract class SwaggerProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        println(roundEnv.getRootElements());
        println("=================================================================");
        println("生成开始>>>");
        SwaggerConfig swaggerConfig = getSwaggerConfig();
        String buildPath = swaggerConfig.getBuildPath();
        if (isBlank(buildPath)) {
            println("未生成文件，路径为空");
            return false;
        }
        String filePath = buildPath + File.separator
                + swaggerConfig.getFileName()
                .replaceAll("\\$\\{projectName}", swaggerConfig.getProjectName())
                .replaceAll("\\$\\{apiVersion}", swaggerConfig.getApiVersion())
                .replaceAll("\\$\\{apiTitle}", swaggerConfig.getApiTitle());
        println("生成文件路径：" + filePath + ".json|yaml");
        Map<String, Object> specification = new LinkedHashMap<>();
        bindProperties(swaggerConfig, specification);
        Map<String, Object> path = new LinkedHashMap<>();
        Set<LinkedHashMap<String, Object>> tagsDesc = new LinkedHashSet<>();
    ...
    private void bindProperties(SwaggerConfig properties, Map<String, Object> spec) {
        ...
        spec.put("info", info);
        if (properties.getServerUrls() != null && properties.getServerUrls().length > 0) {
            List<Map> servers = new ArrayList<>();
            for (String serverUrl : properties.getServerUrls()) {
                servers.add(Collections.singletonMap("url", serverUrl));
            }
